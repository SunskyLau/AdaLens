...
def canonical_ignore_prompt(message: UserMessage) -> str:
    return (message.user_prompt or message.generated_prompt or message.content or "").strip()


def append_ignore_to_turn(turn: Turn, message: UserMessage) -> None:
    prompt = canonical_ignore_prompt(message)
    if prompt:
