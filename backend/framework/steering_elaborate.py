...
def canonical_elaborate_prompt(message: UserMessage) -> str:
    return (message.user_prompt or message.generated_prompt or message.content or "").strip()


def append_elaborate_to_turn(turn: Turn, message: UserMessage) -> None:
    prompt = canonical_elaborate_prompt(message)
    if prompt:
