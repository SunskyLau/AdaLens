...
- Running-plan `modify` uses a two-step control flow: first send `modify` without text to request pause/checkpoint, then submit edited text with a follow-up `modify`; while `launch_requested` is true, modify controls stay disabled.
