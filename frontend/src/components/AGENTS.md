...
- Clicking a `create` steering chat entry must not attempt storyline replay because `create` has no target.
...
- `respond_to_user` agent-response cards should only originate from three backend scenarios: the one-time runtime acknowledgement of a new user-authored `chat` / `focus` / `ignore` / `elaborate` / `create` input, the one-time batch-finished reply when a dispatch batch first becomes fully terminal, or the immediate follow-up to an `evaluate_progress` / `mark_complete` summary.
- That immediate acknowledgement is expected to come from the backend's dedicated fast LLM path using the full canonical user-authored message and steering metadata, not from a truncated excerpt/template shortcut.
- `respond_to_user` agent-response cards may also carry inline citations, and those cards must reuse the exact same inline citation rendering and target-activation logic as stage/final summary cards.
...
- Clicking a stage-summary or final-summary converge button in storyline must first clear storyline selection, then scroll/focus the corresponding chat summary card while `stickToBottom` is turned off before that focus scroll runs. This bridge is specific to `evaluation` / `mark_complete`, not a generic conversation-entry focus system. Converge summary buttons keep hover/pressed emphasis only and must not stay highlighted after click.
- Create-origin active plan areas are the exception: they should still keep plan selection highlighted, but chat focus should additionally highlight and scroll to the corresponding dispatch entry when one exists; only older runs without that dispatch anchor may fall back to the historical `create` steering entry.
- Selecting a directly user-created summary area should keep the normal summary-card chat scroll target and must not add an extra highlight bridge to a `create` steering entry.
...
- New create requests must not generate a separate visible create conversation card. Their visible chat anchor is the relevant `plans_dispatched` entry; older runs that already persisted a create steering card may still show it for compatibility.
...
- Active plan areas must reuse the dispatched-plan card styling from chat, use player-style icon controls for `pause` / `resume` / `terminate`, and stay inside the owning real turn's vertical packing solution instead of reserving a standalone synthetic turn.
